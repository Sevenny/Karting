...
int CProcessZhuang::DoRequest(INPUT_MSG& msg)
{
	...
	CKartingUser* pUser = pTable->GetUserByUserId(msg.uUserId);
	if (!pUser)
	{
	...
	com::game::karting::UserZhuangRequest req;
	if(req.ParseFromArray(msg.pBody, msg.uLen))
	{
		
		return pTable->OnCallZhuang(pUser, req.type());
	}
	return 0;
}
...
///返回0，代表处理成功完成
///返回-1，处理失败
int CProcessZhuang::DoResponse(INPUT_MSG&)
{
	return 0;
}
